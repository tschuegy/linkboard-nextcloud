...
class ProxmoxWidget extends AbstractWidget {
    ...
    public function getConfigFields(): array {
        return [
            [
                'key'               => 'api_token',
                'label'             => 'API Token (PVEAPIToken)',
                'type'              => 'password',
                'required'          => true,
                'placeholder'       => 'user@pam!tokenid=UUID',
                'pattern'           => '^[\w.-]+@\w+![\w.-]+=[\da-fA-F]{8}-[\da-fA-F]{4}-[\da-fA-F]{4}-[\da-fA-F]{4}-[\da-fA-F]{12}$',
                'validationMessage' => 'Expected format: user@realm!tokenid=uuid (e.g. monitor@pam!linkboard=12345678-1234-1234-1234-123456789abc)',
            ],
        ];
    }
